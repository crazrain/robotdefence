...
- **3-to-1 조합**: 동일한 종류와 등급의 영웅 3명이 한 그리드 셀에 모여 있을 때, 플레이어가 **직접 조합 버튼을 눌러** 상위 등급의 영웅 1명으로 진화시킬 수 있습니다. (자동 조합되지 않음)
- **등급 체계**: Basic → Rare → Epic → Legendary → Mythical 순으로 강력해집니다.

### 3. 영웅 이동 및 배치
- **이동 및 스왑**: 영웅을 선택한 후 인접한 빈 셀을 클릭하여 이동하거나, 다른 영웅이 있는 셀을 클릭하여 서로의 위치를 바꿀 수 있습니다.
- **전략적 배치**: 적의 이동 경로와 영웅의 공격 사거리를 고려하여 최적의 위치에 영웅을 배치하는 것이 중요합니다.

### 4. 웨이브 시스템 (Wave System)
- 시간에 따라 적들이 순차적으로 스폰되며, 웨이브가 진행될수록 적의 체력과 속도가 증가합니다.
- 특정 웨이브마다 강력한 보스가 출현합니다.
- 화면 내 살아있는 적의 수가 일정 수(MAX_ALIVE)를 넘어가면 게임 오버됩니다.
- **참고**: 현재 1인 모드(Solo)만 지원하며, **2인 모드(Duo)는 미구현** 상태입니다.

### 5. 업그레이드 및 경제 (Economy)
